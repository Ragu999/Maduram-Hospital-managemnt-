  ...
  node('master'){
    // Run the job in master jenkins
      ...
      stage('Stop IIS website')
      {
        ...
        sh 'ansible-playbook NWC-Dev-stop.yml'
...
stage('publish')
{
  //publish and delete the working dir in jenkins
   bat 'dotnet publish NWC_Creation.sln -c Release -o C:\\inetpub\\wwwroot\\NWCFileCreationSDK'
    deleteDir()
}
}
   stage('service start')
  ...
  node('master'){
    // Run the job in master jenkins
      ...
      stage('Start IIS website')
      {
        ...
        sh 'ansible-playbook NWC-Dev-start.yml'
         deleteDir()
      }
  }
}
