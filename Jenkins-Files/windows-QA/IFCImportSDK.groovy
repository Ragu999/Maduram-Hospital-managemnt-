  ...
  node('master'){
    // Run the job in master jenkins
      ...
      
      {
        ...
        sh 'ansible-playbook ifcimportsdk-QA-stop.yml'
      }
  }
}
node('windows-jobs'){
...
stage('publish')
{
  //publish and delete the working dir in jenkins
   bat 'dotnet publish IFCImportSDK.sln -c Release -o C:\\inetpub\\wwwroot\\IFCImportSDK-QA'
    deleteDir()
}
}
stage('service start')
  ...
  node('master'){
    // Run the job in master jenkins
       ...
       
      {
        ...
        sh 'ansible-playbook ifcimportsdk-QA-start.yml'
         deleteDir()
      }
  }
}
